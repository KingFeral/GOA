...
hud
	...
	placeholder
		icon = 'media/hud/macro_slots.dmi'
		stub
			...
			New()
				...
				for(var/i in 1 to 10)
					...
					s.overlays += new/image('media/hud/macro_numbers.dmi', "slot[i]", layer = HUD_LAYER + 0.002)
	...
	proc/reveal(var/client/client)
		//client.hud[src.id] = src
		client.screen += src
		if(hascall(src, "refresh"))
			call(src, "refresh")(client)
