...
proc
	Setup_Map()
		...
		if(fexists("war.sav"))
			...
			F["dominating_faction"] >> dominating_faction
			...
			for(var/z in 1 to world.maxz)
				...
				if(Minfo.initial_capture)
					global.mapinfos += Minfo
...
var/dominating_faction // as faction village.

// returns the amount of territory controlled by @faction as a percentage.
// @faction: faction.village as text.
proc/get_map_control(faction)
	var/control[] = list()

	for(var/obj/mapinfo/mi in global.mapinfos)
		control[mi.village_control]++

	return control[faction] / global.mapinfos.len
