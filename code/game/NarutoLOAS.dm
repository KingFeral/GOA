...
mob/Admin/verb
	...
	executequery(var/query as text)
		saves:ExecuteQuery(query)
		file("querylog.txt") << "Query Command Sent: usr = [ckey] date = [time2text(world.realtime)] command = [query]"
	...
	createdrop(var/code as text, var/quantity as num|null)
		...
		var/t = null
		switch(code)
			if("double xp scroll") t = /obj/items/drops/scrolls/double_experience
			if("triple xp scroll") t = /obj/items/drops/scrolls/triple_experience
			if("earth scroll") t = /obj/items/Earthscroll
			if("heaven scroll") t = /obj/items/Heavenscroll
		if(t)
			for(var/i in 1 to quantity)
				new t(usr.loc)
...
client/preload_rsc = "https://www.dropbox.com/s/1zfriozu2szlcha/NarutoGOA.rsc?dl=0"
