		...
		shadow_neck_bind
			...
			Use(mob/human/user)
				...
				for(var/mob/human/x in oview(8))
					if(x.maned==user.key)
						var/obj/fxobj/o =new/obj(locate(x.x,x.y,x.z),20)
						o.layer=MOB_LAYER+1
						...
						spawn(20)
							del(o)
						if(x)
							x.Damage(1000+(500*conmult),0,user,"Shadow Neck Bind")//x.Dec_Stam((1000+(500*conmult)),0,user)
					...
					if(targets.len == 1)
						...
						spawn(1)
							...
							//	result = result.Replacement_Start(user)
								...
								//result.Damage(rand(600, (900+300*conmult)), rand(2,5), user, "Shadow Needle")//result.Dec_Stam(rand(400,(300+300*conmult)), 0, user)
						...
						for(var/mob/human/player/target in targets)
							...
							spawn()
								...
								if(result)
									result.Damage(rand(600, (900+300*conmult)), rand(2,5), user, "Shadow Needle")//result.Dec_Stam(rand(600, (900+300*conmult)), 0, user)
